...
public class BaseActivity extends FragmentActivity {
	...
	protected void BaseBackKey(String title,final Activity activity){
        ...
        ImageButton mImageButtonBack = findView(MResource.getIdByName(getApplicationContext(), "id", "button_input_bg_back"));
...
//			this.mProgressDialog = new ProgressDialog(context);
	...
	protected void tipExit() {
		mAlertDialogQuit = new AlertDialog.Builder(this).setTitle("退出").setMessage("您确认要退出 " + this.getString(MResource.getIdByName(getApplicationContext(), "string", "zyqt_app_name")) + " 吗？")
		// 设置自定义对话框的样式
...
//								System.exit(0);
	...
	protected void intentActivity(Activity from, Class<?> to,Bundle bundle,boolean isFinish) {
		...
		if(isFinish){
			from.finish();
			overridePendingTransition(MResource.getIdByName(getApplicationContext(), "anim", "zyqt_slide_in_right"),MResource.getIdByName(getApplicationContext(), "anim", "zyqt_silde_out_left"));
		}
	}
