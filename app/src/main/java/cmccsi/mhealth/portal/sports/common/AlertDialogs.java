...
public class AlertDialogs {
	...
	public static void creatSingleChoiceDialog(String title, Context context, String[] arrayFruit,int defaultitem, final onChoicedTeamListener listener) {
		...
		Dialog dialog = new Builder(context).setTitle(title).setIcon(MResource.getIdByName(context, "drawable", "zyqt_sport_running"))
		  .setPositiveButton("确定", new OnClickListener() {
			@Override
			public void onClick(DialogInterface dialog, int which) {
				listener.onChoicedTeam(choicedTeam);
			}
		}).setSingleChoiceItems(arrayFruit, choicedTeam, new OnClickListener() {
			@Override
			public void onClick(DialogInterface dialog, int which) {
				choicedTeam = which;
			}
