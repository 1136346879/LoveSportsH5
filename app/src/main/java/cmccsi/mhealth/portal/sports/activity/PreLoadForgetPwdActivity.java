...
import cmccsi.mhealth.portal.sports.appversion.WebServiceManage;
...
public class PreLoadForgetPwdActivity extends BaseActivity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(MResource.getIdByName(MapApplication.mContext , "layout", "zyqt_activity_forgetpwd"));
		...
		mEditTextPhoneNum = findView(MResource.getIdByName(MapApplication.mContext , "id", "edittext_phonenum"));
		if (mPhoneNum != null)
	...
	private void initViews() {
		// mLinearLayoutNewPwd = findView(MResource.getIdByName(MapApplication.mContext , "id", "zyqt_a")linearlayout_newpwd);
		// mLinearLayoutNewPwd.setVisibility(View.INVISIBLE);
		userNameEditText = findView(MResource.getIdByName(MapApplication.mContext , "id", "edittext_username"));
		mEditTextPhoneNum = findView(MResource.getIdByName(MapApplication.mContext , "id", "edittext_phonenum"));
		mEditTextTempCode = findView(MResource.getIdByName(MapApplication.mContext , "id", "edittext_tempcode"));
		// mEditTextPassword = findView(MResource.getIdByName(MapApplication.mContext , "id", "zyqt_a")newpwd_edit_setting);
		// mEditTextPasswordAgain = findView(MResource.getIdByName(MapApplication.mContext , "id", "zyqt_a")newpwd_ag_edit_setting);
		mButtonSend = findView(MResource.getIdByName(MapApplication.mContext , "id", "button_send"));
		mButtonOk = findView(MResource.getIdByName(MapApplication.mContext , "id", "button_ok"));
		// mButtonSetPwd = findView(MResource.getIdByName(MapApplication.mContext , "id", "zyqt_a")UpdatePwd_btn_setting);

		mButtonSend.setOnClickListener(this);
		mButtonOk.setOnClickListener(this);
		// mButtonSetPwd.setOnClickListener(this);

		mTextViewTitleBar = (TextView) findViewById(MResource.getIdByName(MapApplication.mContext , "id", "textView_title"));
		mTextViewTitleBar.setText("忘记密码");
		ImageButton back = (ImageButton) findViewById(MResource.getIdByName(MapApplication.mContext , "id", "button_input_bg_back"));
		back.setBackgroundDrawable(getResources().getDrawable(MResource.getIdByName(MapApplication.mContext , "drawable", "zyqt_my_button_back")));
	...
	public synchronized void onClick(View view) {
		if(view.getId()==MResource.getIdByName(MapApplication.mContext , "id", "button_send")){
			...
			sendPwdMessage();
		}else if(view.getId()==MResource.getIdByName(MapApplication.mContext , "id", "button_ok")){
			// 判断短信验证码
			verifyCodeAndJump();
		}
	}
