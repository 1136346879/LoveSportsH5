...
import cmccsi.mhealth.portal.sports.activity.FirmwareDownloadProgressActivity;
import cmccsi.mhealth.portal.sports.activity.FirmwareUpdateProgressActivity;
 ...
 * 设备列表
 * 
 * @type DeviceSettingActivityTest TODO
...
public class DeviceSettingActivityTest extends BaseActivity implements OnClickListener {
	...
	ServiceConnection conn = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName name, IBinder service) {
			...
			// 当前设备如果是手环，检查蓝牙是否开启
			// if (!PreferencesUtils.getString(mContext,
			// SharedPreferredKey.DEVICE_ID, "").isEmpty()
			// && PreferencesUtils.getString(mContext,
			// SharedPreferredKey.DEVICE_ID, "9999").substring(0,
			// 2).equals("01")) {
			// if(Build.VERSION.SDK_INT >= 18){
			// if(checkBlueEnabled()){
			// //连接当前设备
			// mDeviceManagerService.connect(PreferencesUtils.getString(mContext,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(MResource.getIdByName(MapApplication.mContext, "layout", "zyqt_activity_device_setting"));
		...
		new LoadDeviceListTask(getResources().getString(
	...
	private void initViews() {
		...
		mLvDeviceList = (ListView) findViewById(MResource.getIdByName(MapApplication.mContext, "id", "list_device"));

		// 返回按钮
		ImageButton backButton = (ImageButton) findViewById(MResource.getIdByName(MapApplication.mContext, "id", "button_input_bg_back"));
		backButton.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_my_button_back"));
		backButton.setVisibility(View.VISIBLE);
		backButton.setOnClickListener(this);
		// 标题
		title = (TextView) findViewById(MResource.getIdByName(MapApplication.mContext, "id", "textView_title"));
		title.setText("设备管理");
		RelativeLayout rl_add = (RelativeLayout) findViewById(MResource.getIdByName(MapApplication.mContext, "id", "imageButton_title_add"));
		TextView tv_add = (TextView) findViewById(MResource.getIdByName(MapApplication.mContext, "id", "textview_title_add"));
		TextView tv_add_text = (TextView) findViewById(MResource.getIdByName(MapApplication.mContext, "id", "textview_title_add_text"));
		TextView tv_device_empty = (TextView) findViewById(MResource.getIdByName(MapApplication.mContext, "id", "tv_device_setting_text_empty"));
		tv_device_empty.setText(MResource.getIdByName(MapApplication.mContext, "string", "zyqt_text_device_setting_empty"));
//		if (Config.ISALONE) {
		...
		tv_device_empty.setText(MResource.getIdByName(MapApplication.mContext, "string", "zyqt_text_device_setting_empty_alone"));
//		}

		mLlNotice = (LinearLayout) findViewById(MResource.getIdByName(MapApplication.mContext, "id", "ll_device_notice"));
...
//		}
		...
		mAskDialog.setOnDialogCloseListener(new OnDialogCloseListener() {
			...
			public void onClick(int which) {
				if (which == CommonAskDialog.BUTTON_OK) {
					...
					intent.putExtra("downloadsite", mUpdateUrl);
	...
	private void showDeleteInfoDialog(String dialogHeadText, String dialogContent) {
		mDialog = new Dialog(mContext, MResource.getIdByName(MapApplication.mContext, "style", "zyqt_dialog_withStatusBar"));
		mDialog.setContentView(MResource.getIdByName(MapApplication.mContext, "layout", "zyqt_dialog_delete"));
		((TextView) mDialog.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "Introduction_delete"))).setText(dialogHeadText);
		((TextView) mDialog.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "Introduction_delete_detail"))).setText(dialogContent);
		mDialog.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "btn_dialog_delete_yes")).setOnClickListener(new OnClickListener() {
	...
	private void showDeleteAlertDialog(final int position) {
		...
		((TextView) mDialog.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "Introduction_delete"))).setText(dialogHeadText);
		((TextView) mDialog.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "Introduction_delete_detail"))).setText(dialogContent);
		mDialog.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "btn_dialog_delete_yes")).setVisibility(View.GONE);
		mDialog.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "ll_dialog_delete_buttons")).setVisibility(View.VISIBLE);

		mDialog.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "btn_dialog_delete_sure")).setOnClickListener(new OnClickListener() {
		...
		mDialog.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "btn_dialog_delete_cancle")).setOnClickListener(new OnClickListener() {
	...
	private void setSelectedItem() {
		...
		{
			mData.add(0, mSelectedDevice.productName + " " + Build.MODEL);
		} else {
	...
	private void connectDevice(String deviceId) {
		// 当前设备如果是手环，检查蓝牙是否开启
		if (!deviceId.isEmpty() && deviceId.substring(0, 2).equals("01")) {
			if (Build.VERSION.SDK_INT >= 18) {
				if (checkBlueEnabled()) {
					// 连接当前设备
			...
			if (result == 0) {
				if (mDeviceListInfo.datavalue.size() > 0) {
					...
					for (int i = 0; i < mDeviceListInfo.datavalue.size(); i++) {
						int type = Common.getDeviceType(mDeviceListInfo.datavalue.get(i).deviceSerial,
								mDeviceListInfo.datavalue.get(i).productPara);
						if (type == DeviceConstants.DEVICE_MOBILE_STEP)// 手机
						{
							mData.add(mDeviceListInfo.datavalue.get(i).productName + " " + Build.MODEL);
						} else {
							mData.add(mDeviceListInfo.datavalue.get(i).productName + " "
									+ mDeviceListInfo.datavalue.get(i).deviceSerial);
						}
						if (mDeviceListInfo.datavalue.get(i).isUsed.equals("1")) {
							...
							Logger.i(TAG, "---mSelectedDevice " + mSelectedDevice.deviceSerial + " "
									+ mSelectedDevice.productPara);
						}
