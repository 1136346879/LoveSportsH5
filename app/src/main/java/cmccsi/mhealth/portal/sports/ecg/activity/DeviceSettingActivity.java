...
import cmccsi.mhealth.portal.sports.appversion.AddDeviceActivity;
...
import cmccsi.mhealth.portal.sports.device.BaseDeviceInterface;
import cmccsi.mhealth.portal.sports.device.DeviceConstants;
import cmccsi.mhealth.portal.sports.ecg.adapter.DeviceListAdapter;
...
public class DeviceSettingActivity extends BaseActivity implements OnClickListener {
		...
		public void handleMessage(Message msg) {
			...
			case TgbleManagerNeuro.MSG_EXCEPT: // 神念手环连接失败
				...
				if (isTgbleToOther != 1) {
					...
					// 如果从手机计步器切换，失败后重新启动
					if (oldDevice != null
							&& Common.getDeviceType(oldDevice.deviceSerial, oldDevice.productPara) == DeviceConstants.DEVICE_MOBILE_STEP) {
						startPhoneStep();
					}
				} else {
					isUploaded = true;
			...
			case TgbleManagerNeuro.MSG_RESET:
				...
				if (isTgbleToOther != 1) {
					isConnected = true;
					// ToastUtils.showToast(getBaseContext(),
					// msg.getData().getString("MSG_RESET"));
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(MResource.getIdByName(getApplicationContext(), "layout", "zyqt_activity_device_setting"));
		...
		// 提示Dialog
		mDialog = CustomProgressDialog.createDialog(this, true);
		...
		mNewDevicesArrayAdapter = new ArrayAdapter<String>(this, MResource.getIdByName(getApplicationContext(), "layout",
				"zyqt_device_name"));
		mBleMacList = new ArrayList<String>();

		if (Build.VERSION.SDK_INT >= 18 && PreferencesUtils.getBoolean(this, SharedPreferredKey.HAVE_BRACELET_DEVICE, false)) {
			// 初始化手环设备管理工具
	...
	private void initViews() {
		// iv_deviceDrawable = (ImageView)
		// findViewById(MResource.getIdByName(getApplicationContext(), "id",
		// "")iv_deviceDrawable);
		listView = (ListView) findViewById(MResource.getIdByName(getApplicationContext(), "id", "list_device"));

		// 返回按钮
		ImageButton backButton = (ImageButton) findViewById(MResource.getIdByName(getApplicationContext(), "id",
				"button_input_bg_back"));
		backButton.setBackgroundResource(MResource.getIdByName(getApplicationContext(), "drawable", "zyqt_my_button_back"));
		backButton.setVisibility(View.VISIBLE);
		backButton.setOnClickListener(this);
		// 标题
		title = (TextView) findViewById(MResource.getIdByName(getApplicationContext(), "id", "textView_title"));
		title.setText("设备管理");

		RelativeLayout rl_add = (RelativeLayout) findViewById(MResource.getIdByName(getApplicationContext(), "id",
				"imageButton_title_add"));
		TextView tv_add = (TextView) findViewById(MResource.getIdByName(getApplicationContext(), "id", "textview_title_add"));
		TextView tv_add_text = (TextView) findViewById(MResource.getIdByName(getApplicationContext(), "id",
				"textview_title_add_text"));
		TextView tv_device_empty = (TextView) findViewById(MResource.getIdByName(getApplicationContext(), "id",
				"tv_device_setting_text_empty"));
		tv_device_empty.setText(MResource.getIdByName(getApplicationContext(), "string", "zyqt_text_device_setting_empty"));
		// if(Config.ISALONE){
		...
		tv_device_empty.setText(MResource.getIdByName(getApplicationContext(), "string", "zyqt_text_device_setting_empty_alone"));
		// }

		mLlNotice = (LinearLayout) findViewById(MResource.getIdByName(getApplicationContext(), "id", "ll_device_notice"));
		mLlBracelet = (LinearLayout) findViewById(MResource.getIdByName(getApplicationContext(), "id", "ll_bracelet"));
	...
	public void onClick(View v) {
		...
		} else if (id == MResource.getIdByName(getApplicationContext(), "id", "imageButton_title_add")) {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				if (oldDevice != null && oldDevice.deviceSerial.equals(selectedDevice.deviceSerial)) {
					...
					if (oldDeviceType == DeviceConstants.DEVICE_PEDOMETER) {
						showAskDialog("请手动上传您计步器的运动数据，切换后，运动界面将显示" + msg_device_type + "设备的计步数据，且该设备的数据将作为您当天的有效数据参与各排名，您确定切换吗？",
								selectDeviceType);
						...
						if (Build.VERSION.SDK_INT < 18) {
							Toast.makeText(DeviceSettingActivity.this, "您的Android系统版本过低，暂不支持配对该蓝牙手环。", Toast.LENGTH_SHORT).show();
							return;
						}
						showAskDialog("切换后，运动界面将显示" + msg_device_type + "设备的计步数据，且该设备的数据将作为您当天的有效数据参与各排名，您确定切换吗？",
								selectDeviceType);
					} else {
						showAskDialog("切换后，运动界面将显示" + msg_device_type + "设备的计步数据，且该设备的数据将作为您当天的有效数据参与各排名，您确定切换吗？",
								selectDeviceType);
	...
	private void showDeleteInfoDialog(String dialogHeadText, String dialogContent) {
		dialog = new Dialog(mContext, MResource.getIdByName(getApplicationContext(), "style", "zyqt_dialog_withStatusBar"));
		dialog.setContentView(MResource.getIdByName(getApplicationContext(), "layout", "zyqt_dialog_delete"));
		((TextView) dialog.findViewById(MResource.getIdByName(getApplicationContext(), "id", "Introduction_delete")))
				.setText(dialogHeadText);
		((TextView) dialog.findViewById(MResource.getIdByName(getApplicationContext(), "id", "Introduction_delete_detail")))
				.setText(dialogContent);
		dialog.findViewById(MResource.getIdByName(getApplicationContext(), "id", "btn_dialog_delete_yes")).setOnClickListener(
				new OnClickListener() {
	...
	private void showDeleteAlertDialog(final int position) {
		...
		((TextView) dialog.findViewById(MResource.getIdByName(getApplicationContext(), "id", "Introduction_delete_detail")))
				.setText(dialogContent);
		dialog.findViewById(MResource.getIdByName(getApplicationContext(), "id", "btn_dialog_delete_yes")).setVisibility(
				View.GONE);
		dialog.findViewById(MResource.getIdByName(getApplicationContext(), "id", "ll_dialog_delete_buttons")).setVisibility(
				View.VISIBLE);

		dialog.findViewById(MResource.getIdByName(getApplicationContext(), "id", "btn_dialog_delete_sure")).setOnClickListener(
				new OnClickListener() {
		...
		dialog.findViewById(MResource.getIdByName(getApplicationContext(), "id", "btn_dialog_delete_cancle")).setOnClickListener(
				new OnClickListener() {
	...
	private void switchToJWBraclet() {
		...
		if (checkBlueEnabled()) {
			...
			if (old_devcie_type == DeviceConstants.DEVICE_BRACLETE_BEATBAND) {// 神念手环切换到手机，上传数据
				...
				// isConnected = false;
				// mHandlerTemp.postDelayed(_bracelet_connect_timeout,
				// CONNECT_PERIOD);
				isConnected = false;
				mHandlerTemp.postDelayed(_bracelet_connect_timeout, CONNECT_PERIOD);
				showProgress("正在连接丁当手环");
		...
		protected void onPostExecute(Integer result) {
			...
			if (result == 0) {
				...
				editor.putInt(SharedPreferredKey.DEVICE_TYPE,
						Common.getDeviceType(selectedDevice.deviceSerial, selectedDevice.productPara));
				...
				if (result == 500) {
					...
					ToastUtils.showToast(DeviceSettingActivity.this, "网络异常，请检查网络设置！");
				}
				if (oldDevice != null
						&& Common.getDeviceType(oldDevice.deviceSerial, oldDevice.productPara) == DeviceConstants.DEVICE_MOBILE_STEP)// 切换到手机开启手机计步
				{
					startPhoneStep();
				}
				Logger.i(TAG, "---重新加载失败");
			...
			if (result == 0) {
				if (deviceListInfo.datavalue.size() > 0) {
					...
					for (DeviceInfo deviceInfo : deviceListInfo.datavalue) {
						...
						{
							mData.add(deviceInfo.productName + " " + Build.MODEL);
						} else {
							String tempProductName = "";
							tempProductName = Common.getDeviceDisplayName(Common.getDeviceType(deviceInfo.deviceSerial,
									deviceInfo.productPara));
					...
					if (mReload) {
						// ToastUtils.showToast(DeviceSettingActivity.this,
						// "切换成功！");
					}
	...
	private class StepServiceUploadReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(Config.UPLOADSTATUS_ACTION)
				...
				if (!result.equals(getResources().getString(
						MResource.getIdByName(getApplicationContext(), "string", "zyqt_phonestep_uploading")))) {
	...
	private void showMyToast(Context context, String msg) {
		...
		imageCodeProject.setImageResource(MResource.getIdByName(getApplicationContext(), "drawable", "zyqt_toast_star"));
