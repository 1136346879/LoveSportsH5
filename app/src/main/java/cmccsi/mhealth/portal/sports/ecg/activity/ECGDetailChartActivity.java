...
import cmccsi.mhealth.portal.sports.ecg.utils.ChartDataSource;
import cmccsi.mhealth.portal.sports.ecg.utils.DataSourceBuilder;
...
//	private TextView textComment;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(MResource.getIdByName(getApplicationContext(), "layout", "zyqt_activity_ecg_chart"));
		...
		findViewById(MResource.getIdByName(getApplicationContext(), "id", "button_input_bg_back")).setOnClickListener(
				new OnClickListener() {
					@Override
	...
	protected void onResume() {
		super.onResume();
		if (mChartView == null) {
			LinearLayout layout = (LinearLayout) findViewById(MResource.getIdByName(getApplicationContext(), "id", "chart"));
	...
	private void initDialog() {
		dialog = new Dialog(this, MResource.getIdByName(getApplicationContext(), "style", "zyqt_dialog_fullscreen"));
		dialog.setContentView(MResource.getIdByName(getApplicationContext(), "layout", "zyqt_ecg_dialog"));
		...
		dialogRoot = (LinearLayout) dialog.findViewById(MResource.getIdByName(getApplicationContext(), "id", "layout_dialog"));
		textValue = (TextView) dialog.findViewById(MResource.getIdByName(getApplicationContext(), "id", "textview_hr_dialog"));
		textDatatime = (TextView) dialog
				.findViewById(MResource.getIdByName(getApplicationContext(), "id", "textView_datatime_dialog"));
//		textComment = (TextView) dialog
//				.findViewById(MResource.getIdByName(getApplicationContext(), "id", "")textView_comment_dialog);
		View v = dialog.findViewById(MResource.getIdByName(getApplicationContext(), "id", "layout_container"));
	...
	private void showHr(double val, String date) {
		...
		dialogRoot.setBackground(getResources().getDrawable(
				MResource.getIdByName(getApplicationContext(), "drawable", "zyqt_hr_normal")));
//		if (val < 105 && val > 55) {
//			dialogRoot.setBackground(getResources().getDrawable(
...
//		}
	...
	private void showHrv(double val, String date) {
		...
		textDatatime.setText("测量于 " + date);
		dialogRoot.setBackground(getResources().getDrawable(
				MResource.getIdByName(getApplicationContext(), "drawable", "zyqt_hrv_normal")));
//		textComment.setText("心率变异率反映一段时间内的心脏健康指数，平均值越高心脏越健康");
	...
	private void showMood(double val, String date) {
		...
		textDatatime.setText("测量于 " + date);
		dialogRoot.setBackground(getResources().getDrawable(
				MResource.getIdByName(getApplicationContext(), "drawable", "zyqt_stress_normal")));
//		if (val < 50) {
//			dialogRoot.setBackground(getResources().getDrawable(
