package cmccsi.mhealth.portal.sports.service;
...
import cmccsi.mhealth.portal.sports.bean.DataDetailPedo;
...
import cmccsi.mhealth.portal.sports.net.PedometerJsonCreat;
import cmccsi.mhealth.portal.sports.pedo.PedoController;
import cmccsi.mhealth.portal.sports.phonesteplib.SPUtils;
import cmccsi.mhealth.portal.sports.phonesteplib.StepDetector;
import cmccsi.mhealth.portal.sports.phonesteplib.UserInstance;
...
public class StepService extends Service {
	...
	// "cmccsi.mhealth.portal.sports.RESUME_PEDOACTIVITY";
	// 发送消息的数据
	...
	private void showNotification(String step, String cal, String percent) {
		RemoteViews remoteView = new RemoteViews(this.getPackageName(), MResource.getIdByName(MapApplication.mContext, "layout",
				"zyqt_notification_phone_step"));
		remoteView.setTextViewText(MResource.getIdByName(MapApplication.mContext, "id", "tv_stepsum"), step + "步");
		remoteView.setTextViewText(MResource.getIdByName(MapApplication.mContext, "id", "tv_cal"), cal + "千卡");
		remoteView.setTextViewText(MResource.getIdByName(MapApplication.mContext, "id", "tv_percent"), percent + "%");
		Intent intent = new Intent(StepService.this,
				cmccsi.mhealth.portal.sports.pedo.PedometerActivity.class);
		intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
		PendingIntent contentIntent = PendingIntent.getActivity(StepService.this, 0, intent,
				PendingIntent.FLAG_CANCEL_CURRENT);
		noti = new Notification.Builder(this)
				.setSmallIcon(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_icon_notify_ticker"))
				.setContent(remoteView).setContentIntent(contentIntent).getNotification();
	...
	public void notifyNotificationPercent() {
		...
		RemoteViews remoteView = new RemoteViews(this.getPackageName(), MResource.getIdByName(MapApplication.mContext, "layout",
				"zyqt_notification_phone_step"));
		remoteView.setTextViewText(MResource.getIdByName(MapApplication.mContext, "id", "tv_stepsum"), todayTotalStep + "步");
		remoteView.setTextViewText(MResource.getIdByName(MapApplication.mContext, "id", "tv_cal"), todayTotalCal + "千卡");
				...
				if (dataFromPHP == 0) {
					intent.putExtra(
							...
							getResources().getString(
									MResource.getIdByName(MapApplication.mContext, "string", "zyqt_phonestep_uploadsuccess")));
				} else {
					intent.putExtra(
							UPLOAD_STATUS,
							getResources().getString(
									MResource.getIdByName(MapApplication.mContext, "string", "zyqt_phonestep_uploadfailed")));
				}
	...
	private void sendUpLoadIntent() {
		sendIntentBroadcast(Config.PHONESTEP_STARTUPLOAD_ACTION, UPLOAD_STATUS,
				getResources().getString(MResource.getIdByName(MapApplication.mContext, "string", "zyqt_phonestep_uploading")));
	}
	...
	public static boolean isServiceRunning(Context mContext) {
		...
		// if
		// (serviceList.get(i).service.getClassName().equals(StepService.class.getName())
		// == true) {
		// isRunning = true;
		// break;
		// }
