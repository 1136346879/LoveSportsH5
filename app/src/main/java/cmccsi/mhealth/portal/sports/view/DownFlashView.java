...
//	private boolean isRefreshEnabled = true;
	...
	private void init() {
		...
		// 刷新视图顶端的的view
		refreshView = LayoutInflater.from(mContext).inflate(MResource.getIdByName(mContext, "layout", "zyqt_refresh_top_item"), null);
		// 指示器view
		refreshIndicatorView = (ImageView) refreshView.findViewById(MResource.getIdByName(mContext, "id", "indicator"));
		refreshIndicatorView.setImageResource(MResource.getIdByName(mContext, "drawable", "zyqt_arrow"));
		// 刷新bar
		bar = (ProgressBar) refreshView.findViewById(MResource.getIdByName(mContext, "id", "progress"));
		bar.setVisibility(View.GONE);
		// 下拉显示text
		downTextView = (TextView) refreshView.findViewById(MResource.getIdByName(mContext, "id", "refresh_hint"));
		// 下来显示时间
		timeTextView = (TextView) refreshView.findViewById(MResource.getIdByName(mContext, "id", "refresh_time"));

		LayoutParams lp = new LayoutParams(LayoutParams.MATCH_PARENT,(int) (60*Common.getDensity(getContext())));
...
//		this.isRefreshEnabled = b;
	...
	public void setState(int state) {
		...
		if (state == STATE_REFRESHING) { // 显示进度
			...
			downTextView.setText(getResources().getString(MResource.getIdByName(mContext, "string", "zyqt_xlistview_header_hint_loading")));
		} else { // 显示箭头图片
		...
		case STATE_NORMAL:
			if (mState == STATE_READY) {
				downTextView.setText(getResources().getString(MResource.getIdByName(mContext, "string", "zyqt_xlistview_header_hint_normal")));
		...
		case STATE_READY:
			if (mState != STATE_READY) {
				downTextView.setText(getResources().getString(MResource.getIdByName(mContext, "string", "zyqt_xlistview_header_hint_ready")));
				refreshIndicatorView.clearAnimation();
