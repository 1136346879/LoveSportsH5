package cmccsi.mhealth.portal.sports.device;
...
//				clearDeviceData();
	...
	private BaseCallBack mBaseCallBack = new BaseCallBack(){
		...
		protected void onPostExecute(Integer result) {
			...
			if (result == 0) {
				...
				if (result == 500) {
					...
					Logger.i("cjz", "网络异常！");
					resultStr = getResources().getString(MResource.getIdByName(MapApplication.mContext, "string", "zyqt_MESSAGE_INTERNET_ERROR"));
				}
				message = Message.obtain(mHandlerActivity, DeviceConstants.DEVCIE_SWITCH_FAIL);
