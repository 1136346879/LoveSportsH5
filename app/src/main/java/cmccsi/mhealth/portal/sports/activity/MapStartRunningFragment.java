...
import cmccsi.mhealth.portal.sports.basic.SampleFragment;
...
public class MapStartRunningFragment extends SampleFragment implements OnClickListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		view = inflater.inflate(MResource.getIdByName(MapApplication.mContext, "layout", "zyqt_acticity_running"), container,
				false);
	...
	public void findViews() {
		...
		mFinishRunnnig = (Button) findView(MResource.getIdByName(MapApplication.mContext, "id", "button_finish_running"));
		mStopRunnnig = (Button) findView(MResource.getIdByName(MapApplication.mContext, "id", "button_stop_running"));
		mTextViewkilometre = findView(MResource.getIdByName(MapApplication.mContext, "id", "textview_kilometre"));
		...
		mLlayoutMap = findView(MResource.getIdByName(MapApplication.mContext, "id", "LLayout_map"));

		mTextViewTimer = findView(MResource.getIdByName(MapApplication.mContext, "id", "startmap_timer"));
		if (mTextviewTime != null && StepService_GPS.isServiceStart) {
		...
		mButtonMoveToLocation = findView(MResource.getIdByName(MapApplication.mContext, "id", "button_get_location"));
		mButtonOpenMap = findView(MResource.getIdByName(MapApplication.mContext, "id", "button_open_map"));
		mIvMapGps = findView(MResource.getIdByName(MapApplication.mContext, "id", "iv_map_gps"));

		if (mStart_cal1 != null && StepService_GPS.isServiceStart) {
		...
		mTextViewAltitede = findView(MResource.getIdByName(MapApplication.mContext, "id", "textview_altitude"));
		// mTextViewCal = findView(MResource.getIdByName(MapApplication.mContext
		// , "id", "zyqt_a")textview_cal);
		...
		// 添加标注点覆盖物
		mOverlayStartItem = BitmapDescriptorFactory.fromResource(MResource.getIdByName(MapApplication.mContext, "drawable",
				"zyqt_img_map_start"));
	...
	StepService_GPS.OnMyLocationChangeListener changeListener = new StepService_GPS.OnMyLocationChangeListener() {
		...
		public void gpsIntensity(int intensity) {
			...
			if (intensity == 0) {
				mIvMapGps.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_map_gps_q"));
			} else {
				mIvMapGps.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_map_gps_r"));
			}
...
//	}
	...
	private void setOnMarkClickListener() {
		mBaiduMap.setOnMarkerClickListener(new OnMarkerClickListener() {
			...
			public boolean onMarkerClick(final Marker marker) {
				...
				button.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_popup"));
	...
	public void onDestroy() {
		...
		// PreferencesUtils.putString(mActivity, SharedPreferredKey.TIMER,
		...
		this.getView().setOnKeyListener(new OnKeyListener() {
			...
			public boolean onKey(View v, int keyCode, KeyEvent event) {
				...
				if (event.getAction() == KeyEvent.ACTION_UP && keyCode == KeyEvent.KEYCODE_BACK) {
					ToastUtils.showToast(getActivity(), "轨迹记录结束后才可退出");
					return true;
				} else
					return false;
			}
	...
	public void onClick(View v) {
		...
		if (v.getId() == MResource.getIdByName(MapApplication.mContext, "id", "button_finish_running")) {
			mFinishRunnnig.setEnabled(false);
			saveData();
			mFinishRunnnig.setEnabled(true);
		} else if (v.getId() == MResource.getIdByName(MapApplication.mContext, "id", "button_stop_running")) {
			if (mStart_Stop) {
		...
		} else if (v.getId() == MResource.getIdByName(MapApplication.mContext, "id", "button_input_bg_back")) {
			...
			if (starttime != null) {
				...
				handler.sendEmptyMessage(MResource.getIdByName(MapApplication.mContext, "string", "zyqt_MESSAGE_UN_SAVE_DATA"));
			}
			getActivity().finish();
		} else if (v.getId() == MResource.getIdByName(MapApplication.mContext, "id", "map_start")) {
			mMapView.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
		} else if (v.getId() == MResource.getIdByName(MapApplication.mContext, "id", "button_open_map")) {
			openOrCloseMap(mWidth, mHeight, windowWidth, windowheight);
		} else if (v.getId() == MResource.getIdByName(MapApplication.mContext, "id", "button_get_location")) {
...
//		}
	...
	private void saveData() {
		...
		if (MHealthProviderMetaData.GetMHealthProvider(mActivity).getGpsInfoDetails(starttime).size() < 2) {
			...
			mAskDialog = CommonAskDialog.create("你还没有运动轨迹，确定要退出吗？", buttons, false, true);
			...
			mAskDialog.setOnDialogCloseListener(new OnDialogCloseListener() {
				...
				public void onClick(int which) {
					if (which == CommonAskDialog.BUTTON_OK) {
						if (starttime != null) {
							MHealthProviderMetaData.GetMHealthProvider(mActivity).deleteDetailData(starttime);
							handler.sendEmptyMessage(MResource.getIdByName(MapApplication.mContext, "string",
									"zyqt_MESSAGE_UN_SAVE_DATA"));
						}
						finishRunning();
						// if (tag.equals("map")) {
						// FragmentManager sfm =
			...
			mAskDialog = CommonAskDialog.create("是否保存您的轨迹数据？", buttons, true, true);
			mAskDialog.setAlertIconVisible(-1);
			mAskDialog.setOnDialogCloseListener(new OnDialogCloseListener() {
				@Override
				public void onClick(int which) {
	...
	Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			if (msg.what == MResource.getIdByName(MapApplication.mContext, "string", "zyqt_MESSAGE_UPLOAD_GPS_SUCCESS")) {
				BaseToast(String.valueOf(getString(MResource.getIdByName(MapApplication.mContext, "string",
						"zyqt_MESSAGE_UPLOAD_GPS_SUCCESS"))));
				updateDataBase(msg, 0);
			} else if (msg.what == MResource.getIdByName(MapApplication.mContext, "string", "zyqt_MESSAGE_UN_SAVE_DATA")) {
				removeSp();
				getActivity().finish();
			} else if (msg.what == TIME_CHANGE) {
		...
		private void updateDataBase(Message msg, int flag) {
	...
	private void openOrCloseMap(int mWidth, int mHeight, int wWidth, int wHeight) {
		...
		// }
		if (mOpenMap) {
			mButtonOpenMap.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_map_max_min"));
			...
			mButtonOpenMap.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_map_min"));
...
//		BaseToast("已停止运动");
	...
	public void showNotification() {
		// Intent intent = new Intent(
		// cmccsi.mhealth.portal.sports.service.StepService_GPS.this,
		// cmccsi.mhealth.portal.sports.pedo.PedometerActivity.class);
		// intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
		// PendingIntent contentIntent = PendingIntent.getActivity(
		// cmccsi.mhealth.portal.sports.service.StepService.this, 0,
		// intent, PendingIntent.FLAG_CANCEL_CURRENT);
		//
		//
		// Intent appIntent = new Intent(Intent.ACTION_MAIN);
		Intent appIntent = new Intent(getActivity().getApplicationContext(),
				cmccsi.mhealth.portal.sports.activity.FragmentContainerActivity.class);
		// appIntent.addCategory(Intent.CATEGORY_LAUNCHER);
		// appIntent.setComponent(new
		// ComponentName(getActivity().getApplicationContext(),"cmccsi.mhealth.portal.sports.activity."
		// + getActivity().getLocalClassName()));

		// Intent appIntent = new Intent(getActivity(),
		...
		RemoteViews contentView = new RemoteViews(getActivity().getPackageName(), MResource.getIdByName(MapApplication.mContext,
				"layout", "zyqt_notification_phone_sendmsg"));
		contentView.setTextViewText(MResource.getIdByName(MapApplication.mContext, "id", "notification_phone_sendmsg_text"),
				"轨迹记录正在进行");
		contentView.setTextViewTextSize(MResource.getIdByName(MapApplication.mContext, "id", "notification_phone_sendmsg_text"),
				TypedValue.COMPLEX_UNIT_SP, 18);
		Notification notification = new NotificationCompat.Builder(getActivity())
				.setSmallIcon(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_i_shang")).setContent(contentView)
				.setContentIntent(contentIntent).setAutoCancel(true).setDefaults(Notification.DEFAULT_ALL).build();
