...
import cmccsi.mhealth.portal.sports.view.picker.TimePicker;

public class BraceletSettingActivity extends BaseActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(MResource.getIdByName(getApplicationContext(), "layout", "zyqt_activity_bracelet_setting"));
		initViews();
		loadConfig();
	}
	...
	private void loadConfig() {		
		...
		boolean enableSport = sp.getBoolean(SharedPreferredKey.ENABLE_SPORT,
				false);
		boolean enableSleep = sp.getBoolean(SharedPreferredKey.ENABLE_SLEEP,
				false);
		int sportRepeat = sp.getInt(SharedPreferredKey.DAYS, -1);
		...
		loadRepeat(llSleep, sportRepeat);
	...
	private void saveConfig() {
		...
		int sportRepeat = getRepeatConfig(llSleep);
		...
		e.putInt(SharedPreferredKey.DAYS, sportRepeat);
	...
	private int getRepeatConfig(LinearLayout container) {
		...
		for (int i = 0; i < container.getChildCount(); i++) {
			...
			buffer.append(cb.isChecked() ? "0" : "1");
		}
		buffer.append("1");
	...
	private void initViews() {
		...
		tpSport = (TimePicker) findViewById(MResource.getIdByName(getApplicationContext(), "id", "tp_sport"));
		tpSleep = (TimePicker) findViewById(MResource.getIdByName(getApplicationContext(), "id", "tp_sleep"));
		llSleep = (LinearLayout) findViewById(MResource.getIdByName(getApplicationContext(), "id", "ll_sleep"));
		tbSport = (ToggleButton) findViewById(MResource.getIdByName(getApplicationContext(), "id", "tb_sport"));
		tbSleep = (ToggleButton) findViewById(MResource.getIdByName(getApplicationContext(), "id", "tb_sleep"));

		tbSport.setOnCheckedChangeListener(this);
		tbSleep.setOnCheckedChangeListener(this);

		rbLeft = (RadioButton) findViewById(MResource.getIdByName(getApplicationContext(), "id", "radio_left"));
		rbRight = (RadioButton) findViewById(MResource.getIdByName(getApplicationContext(), "id", "radio_right"));

		findViewById(MResource.getIdByName(getApplicationContext(), "id", "btn_save")).setOnClickListener(this);
	}
	...
	public void onClick(View v) {
		...
		if (id == MResource.getIdByName(getApplicationContext(), "id", "button_input_bg_back")) {
			this.finish();
		} else if (id == MResource.getIdByName(getApplicationContext(), "id", "btn_save")) {
	...
	public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
		...
		if (id == MResource.getIdByName(getApplicationContext(), "id", "tb_sport")) {
			tpSport.setEnabled(isChecked);
		} else if (id == MResource.getIdByName(getApplicationContext(), "id", "tb_sleep")) {
