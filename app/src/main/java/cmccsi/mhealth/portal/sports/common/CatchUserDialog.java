...
public class CatchUserDialog {
	...
	private Handler handle = new Handler() {
		...
		public void dispatchMessage(Message msg) {
			...
			case FIND_SUCCESS:
				...
				mInvite.setBackgroundResource(MResource.getIdByName(context, "drawable", "zyqt_sample_usercatch_button_ok_bg"));
			...
			case FIND_MYSELF:
                ...
                tvtip.setText(context
                        .getString(MResource.getIdByName(context, "string", "zyqt_text_addfriend_find_myself")));
			    break;
			case NET_PROBLEM:
				if (listener != null) {
                    listener.onCapturedUser(CAPTURE_FAIL, context.getResources().getString(MResource.getIdByName(MapApplication.mContext, "string", "zyqt_MESSAGE_INTERNET_ERROR")), null,
                            null, null);
				}
				resetFriendInfo();
				break;
	...
	private void resetFriendInfo() {
		...
		mInvite.setTextColor(context.getResources().getColor(MResource.getIdByName(context, "color", "zyqt_green_friend")));
		mInvite.setBackgroundResource(MResource.getIdByName(context, "drawable", "zyqt_btn_canclefriend"));
		mFriendAvatar.setImageDrawable(context.getResources().getDrawable(MResource.getIdByName(context, "drawable", "zyqt_tab_add_friend")));
		mFriendAvatar.setBackgroundResource(MResource.getIdByName(context, "drawable", "zyqt_tab_add_friend"));
		tvtip.setText("");
	}
	...
	public void startCapture(String okbtnStr) {
		...
		customDialog = new Dialog(context, MResource.getIdByName(context, "style", "zyqt_dialog_withStatusBar"));
		customDialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
		customDialog.setCanceledOnTouchOutside(true);
		View viewDialog = View.inflate(context, MResource.getIdByName(context, "layout", "zyqt_alertdialog_addfriend"), null);

		inputPhone = (EditText) viewDialog.findViewById(MResource.getIdByName(context, "id", "af_etn_keyword"));
		...
		mCancel = (Button) viewDialog.findViewById(MResource.getIdByName(context, "id", "af_btn_canclefriend"));
		...
		mInvite = (Button) viewDialog.findViewById(MResource.getIdByName(context, "id", "af_btn_addfriend"));
		...
		mFriendAvatar = (ImageView) viewDialog.findViewById(MResource.getIdByName(context, "id", "af_avatar_icon"));
		mFriendAvatar.setBackgroundResource(MResource.getIdByName(context, "drawable", "zyqt_tab_add_friend"));
		
		TextView cmtdsd = (TextView) viewDialog.findViewById(MResource.getIdByName(context, "id", "af_click_me_to_dismiss_search_dialog"));
		...
		tvtip = (TextView) viewDialog.findViewById(MResource.getIdByName(context, "id", "af_tip"));
		tvtip.setVisibility(View.INVISIBLE);

		Window window = customDialog.getWindow();
		window.setGravity(Gravity.BOTTOM); // 
		window.setWindowAnimations(MResource.getIdByName(context, "style", "zyqt_mystylebottom")); // 添加动画
	...
	private void findFriend(final String str) {
	    mFriendAvatar.setImageDrawable(context.getResources().getDrawable(MResource.getIdByName(context, "drawable", "zyqt_unknowfriend")));
