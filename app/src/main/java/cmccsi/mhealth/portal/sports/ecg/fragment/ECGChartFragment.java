package cmccsi.mhealth.portal.sports.ecg.fragment;
...
import cmccsi.mhealth.portal.sports.common.MathUtil;
import cmccsi.mhealth.portal.sports.db.MHealthProviderMetaData;
import cmccsi.mhealth.portal.sports.ecg.activity.ECGDetailChartActivity;
import cmccsi.mhealth.portal.sports.ecg.utils.ECGDataFilter;
...
public class ECGChartFragment extends Fragment implements OnClickListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		View view = inflater.inflate(MResource.getIdByName(MapApplication.mContext, "layout", "zyqt_fragment_ecg_detail"), container, false);
		charHR = (StaticSplineChartView) view.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "chart_hr"));
		charHRV = (StaticSplineChartView) view.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "chart_hrv"));
		charMOOD = (StaticSplineChartView) view.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "chart_mood"));
	...
	private void bindLatestAndAverage(List<DataECG> ecgDataList) {
		...
		((TextView) root.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "textView_hr_av"))).setText(String.format("%.1f", hrTotal / size));
		((TextView) root.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "textView_hrv_av"))).setText(String.format("%.1f", hrvTotal / size));
		((TextView) root.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "textView_stress_av"))).setText(String.format("%.1f", moodTotal / size));

		((TextView) root.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "textView_hr"))).setText(ecgDataList.get(size - 1).data.hr);
		((TextView) root.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "textView_hrv"))).setText(ecgDataList.get(size - 1).data.hrv);
		((TextView) root.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "textView_stress"))).setText(ecgDataList.get(size - 1).data.mood);
	}

	@Override
	public void onClick(View v) {
		int id = v.getId();
		if (id == MResource.getIdByName(MapApplication.mContext, "id", "chart_hr") || id == MResource.getIdByName(MapApplication.mContext, "id", "chart_hrv") || id == MResource.getIdByName(MapApplication.mContext, "id", "chart_mood")) {
			StaticSplineChartView chart = (StaticSplineChartView) v;
			if (chart.hasData()) {
				Intent intent = new Intent(getActivity(), ECGDetailChartActivity.class);
				int type = (id == MResource.getIdByName(MapApplication.mContext, "id", "chart_hr") ? ECGDetailChartActivity.TYPE_HR
						: (id == MResource.getIdByName(MapApplication.mContext, "id", "chart_hrv") ? ECGDetailChartActivity.TYPE_HRV : ECGDetailChartActivity.TYPE_MOOD));
