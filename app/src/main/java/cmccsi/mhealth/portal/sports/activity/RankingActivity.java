...
import cmccsi.mhealth.portal.sports.bean.PedoRankBriefInfo;
import cmccsi.mhealth.portal.sports.bean.PedoRankDetailInfo;
...
import cmccsi.mhealth.portal.sports.controller.PedoRankController;
import cmccsi.mhealth.portal.sports.net.DataSyn;
import cmccsi.mhealth.portal.sports.tabhost.RankinglistFragment;
...
public class RankingActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(MResource.getIdByName(MapApplication.mContext , "layout", "zyqt_activity_ranking"));
...
//					mFtList.clear();
	...
	private void findViewById() {
		...
		mTvTitle =  (TextView) findViewById(MResource.getIdByName(MapApplication.mContext , "id", "textView_title_rank"));
		
		mIvBack = (ImageView) findViewById(MResource.getIdByName(MapApplication.mContext , "id", "button_input_bg_back"));

		rl_nav = (RelativeLayout) findViewById(MResource.getIdByName(MapApplication.mContext , "id", "rl_nav"));
		mRgTime = (RadioGroup) findViewById(MResource.getIdByName(MapApplication.mContext , "id", "rg_time"));
		mRgPersonalOrTeam = (RadioGroup) findViewById(MResource.getIdByName(MapApplication.mContext , "id", "rg_personal_team"));

		mHsv = (SyncHorizontalScrollView) findViewById(MResource.getIdByName(MapApplication.mContext , "id", "mHsv"));
		rg_nav_content = (RadioGroup) findViewById(MResource.getIdByName(MapApplication.mContext , "id", "rg_nav_content"));
		iv_nav_indicator = (ImageView) findViewById(MResource.getIdByName(MapApplication.mContext , "id", "iv_nav_indicator"));
		iv_nav_left = (ImageView) findViewById(MResource.getIdByName(MapApplication.mContext , "id", "iv_nav_left"));
		iv_nav_right = (ImageView) findViewById(MResource.getIdByName(MapApplication.mContext , "id", "iv_nav_right"));
		mViewPager = (ViewPager) findViewById(MResource.getIdByName(MapApplication.mContext , "id", "mViewPager"));

		mRgTime.setOnCheckedChangeListener(new OnCheckedChangeListener() {

			@Override
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				if (checkedId == MResource.getIdByName(MapApplication.mContext , "id", "rb_yestoday")) {
					oneOrSeven = "1";
					i = 1;
				} else if (checkedId == MResource.getIdByName(MapApplication.mContext , "id", "rb_sevendays")) {
					oneOrSeven = "7";
					i = 1;
				}
				getRankInfo();
		...
		mRgPersonalOrTeam.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				...
				if (checkedId == MResource.getIdByName(MapApplication.mContext , "id", "rb_personal")) {
					teamOrPerSon = "user";
					// currentRBtnClk=1;
				} else if (checkedId == MResource.getIdByName(MapApplication.mContext , "id", "rb_team")) {
					teamOrPerSon = "group";
					// currentRBtnClk=2;
				}
				getRankInfo();
	...
	private void initNavigationHSV() {
		...
		// 添加选项卡
		for (int i = 0; i < tabTitle.size(); i++) {
			RadioButton rb = (RadioButton) mInflater.inflate(MResource.getIdByName(MapApplication.mContext , "layout", "zyqt_nav_radiogroup_item"), null);
			rb.setId(i);
	...
	private class GetPedoRankListInfoTask extends AsyncTask<String, Void, Integer> {
		...
		protected void onPreExecute() {
			showProgressDialog(getResources().getString(MResource.getIdByName(MapApplication.mContext, "string", "zyqt_text_wait")), RankingActivity.this);
			super.onPreExecute();
		}
