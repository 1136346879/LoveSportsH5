...
public class TgbleManagerNeuro {
	...
	public void content() {
		...
		mDeviceAdress = mDeviceAdress.equals("") ? PreferencesUtils.getString(mContext, SharedPreferredKey.DEVICE_ADDRESS, null) : mDeviceAdress;
		Logger.d(TAG, "---mDeviceAdress " + mDeviceAdress);
...
//					msgresult="手环连接超时";
			...
			if(msgresult!=""&&!msgresult.equals("")&&!msgresult.equals("5"))
			{
				if(mAlertHandler!=null)
				{
					Message msg = Message.obtain(mAlertHandler, MSG_EXCEPT);
...
//			connectStatus=0;
	...
	class UploadPedoAsk extends AsyncTask<Null, Null, Integer> {
		...
		protected void onPostExecute(Integer result) {
			...
			if(mAlertHandler!=null&&result==-1)
			{
				Message msg;
