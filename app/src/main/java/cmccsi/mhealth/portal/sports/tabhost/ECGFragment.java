...
import cmccsi.mhealth.portal.sports.bean.SimplePostInfo;
...
import cmccsi.mhealth.portal.sports.net.EcgJsonCreat;
import cmccsi.mhealth.portal.sports.pedo.TgbleManagerNeuro;
import cmccsi.mhealth.portal.sports.view.CenterRollingBallECG;
...
public class ECGFragment extends BaseFragment {
	...
	private TgbleManagerNeuro tgble;
	...
	Handler mbleAlertHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case TgbleManagerNeuro.MSG_EXCEPT:
				...
				closeProgress();
				setBleConnectFailState();
				ToastUtils.showToast(getActivity(), edata.getString("MSG_EXCEPT"));
				break;
			case TgbleManagerNeuro.MSG_RESET:
				...
				closeProgress();
				setBleConnectFailState();
				ToastUtils.showToast(getActivity(), edataReset.getString("MSG_RESET"));
				break;
			case TgbleManagerNeuro.MSG_CONNECTED:
				...
				setBleConnectSucessState();
				break;
			case TgbleManagerNeuro.MSG_STATUS:
				Bundle data1 = msg.getData();
				String status = data1.getString("MSG_STATUS");
				if (status.startsWith("正在测量")) {
					...
					Toast.makeText(getActivity(), data1.getString("MSG_STATUS"), Toast.LENGTH_SHORT).show();
				}
				break;
			...
			case TgbleManagerNeuro.MSG_FAILED:
				ToastUtils.showToast(getActivity(), msg.getData().getString("MSG_FAILED"));
				break;
			case TgbleManagerNeuro.MSG_EKGSTOP:
				ekgStop(msg.getData().getInt("EKGSTOP_RESULT"), msg.getData().getInt("EKGSTOP_FINALHR"));
				break;
			case TgbleManagerNeuro.MSG_DATARECEIVED:
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		View view = inflater.inflate(MResource.getIdByName(MapApplication.mContext, "layout", "zyqt_activity_ecg_new"), container, false);
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		tgble = TgbleManagerNeuro.getSingleInstance(getActivity().getApplicationContext());
		tgble.setHandle(mbleAlertHandler);

		if (resultCode == Activity.RESULT_OK && requestCode == REQUEST_ENABLE_BT) {
			tgble.content();
			// handler.removeCallbacks(_l_b_fail_scan);
			// handler.postDelayed(_l_b_fail_scan, SCAN_PERIOD);
		} else {
			BaseToast("蓝牙未开启");
		}
	}
		...
		public void handleMessage(Message msg) {
			...
			case MSG_HR:
				...
				if (data1.getInt("HR_STATUS") == 0) {
					mHRProgress.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_display_2"));
				} else {
					mHRProgress.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_countdown_ring_22"));
				}
				break;
			case MSG_HRV:
			...
			case MSG_MOOD:
				...
				if (dataMood < 20) {
					ecg_level.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_ecg_level2"));
				} else if (dataMood < 40) {
					ecg_level.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_ecg_level3"));
				} else if (dataMood < 60) {
					ecg_level.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_ecg_level4"));
				} else if (dataMood < 80) {
					ecg_level.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_ecg_level5"));
				} else {
					ecg_level.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_ecg_level6"));
				}
	...
	private void initDialog(String dialogHeadText, String dialogContent) {
		dialog = new Dialog(mActivity, MResource.getIdByName(MapApplication.mContext, "style", "zyqt_dialog_fullscreen"));
		dialog.setContentView(MResource.getIdByName(MapApplication.mContext, "layout", "zyqt_dialog_hrv"));
		((TextView) dialog.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "Introduction_hrv"))).setText(dialogHeadText);
		((TextView) dialog.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "Introduction_hrv_detail"))).setText(dialogContent);
		...
		dialog.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "rl_container")).setOnClickListener(new OnClickListener() {
			@Override
	...
	private void showDeviceDialog() {
		dialog.findViewById(MResource.getIdByName(MapApplication.mContext, "id", "Introduction_hrv")).setVisibility(View.VISIBLE);
		dialog.show();
	}

	public void initialView() {
		// title
		buttonText = (TextView) findView(MResource.getIdByName(MapApplication.mContext, "id", "buttonText"));
		img_heart = findView(MResource.getIdByName(MapApplication.mContext, "id", "img_heart"));
		big_animation = AnimationUtils.loadAnimation(mActivity, MResource.getIdByName(MapApplication.mContext, "anim", "zyqt_big_heart"));

		tv_RR_length = (TextView) findView(MResource.getIdByName(MapApplication.mContext, "id", "textview_rr"));
		tv_RR_length2 = (TextView) findView(MResource.getIdByName(MapApplication.mContext, "id", "textview_rr_2"));
		// tv = (TextView) findView(MResource.getIdByName(MapApplication.mContext, "id", "zyqt_activity_weight")textView_sdk_version);
		...
		tv_HRV = (TextView) findView(MResource.getIdByName(MapApplication.mContext, "id", "textview_hrv"));
		// ECGResult = (TextView) findView(MResource.getIdByName(MapApplication.mContext, "id", "zyqt_activity_weight")text_mbwcl);
		tv_HR = (TextView) findView(MResource.getIdByName(MapApplication.mContext, "id", "textview_HR_BPM2"));

		ecg_level = (ImageView) findView(MResource.getIdByName(MapApplication.mContext, "id", "ecg_level"));

		tv_TraningZone = (TextView) findView(MResource.getIdByName(MapApplication.mContext, "id", "textview_heartlevel"));
		tv_TraningZone2 = (TextView) findView(MResource.getIdByName(MapApplication.mContext, "id", "textview_heartlevel_2"));
		mHRbadresult = (RelativeLayout) findView(MResource.getIdByName(MapApplication.mContext, "id", "Progress_hrv"));
		...
		mProgress_heartlevel = (RelativeLayout) findView(MResource.getIdByName(MapApplication.mContext, "id", "Progress_heartlevel"));
		...
		mProgress_rr = (RelativeLayout) findView(MResource.getIdByName(MapApplication.mContext, "id", "Progress_rr"));
		...
		mHRProgressResult = (RelativeLayout) findView(MResource.getIdByName(MapApplication.mContext, "id", "Progress_hr_result"));
		...
		layout = (RelativeLayout) findView(MResource.getIdByName(MapApplication.mContext, "id", "ecg_view_line"));
		...
		mRelativeLayoutProgress = findView(MResource.getIdByName(MapApplication.mContext, "id", "Progress_center_rote2"));
		// gvSeries1.appendData(new GraphViewData(graph2LastXValue, 0), true,
		// ECGlength);

		mHRProgress = (RelativeLayout) findView(MResource.getIdByName(MapApplication.mContext, "id", "rel_center_progress"));

		mImageButtonBack = (ImageButton) findView(MResource.getIdByName(MapApplication.mContext, "id", "button_input_bg_back"));
		mImageButtonBack.setVisibility(View.VISIBLE);
		// mImageButtonBack.setBackgroundDrawable(mActivity.getResources().getDrawable(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_activity_weight")slidemenu_button));
		...
		mImageButtonBack.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				// m.showMenu();
				// overridePendingTransition(MResource.getIdByName(MapApplication.mContext, "anim", "zyqt_activity_weight")slide_in_left,MResource.getIdByName(MapApplication.mContext, "anim", "zyqt_activity_weight")silde_out_right);
				getActivity().finish();
			}
		});

		rlStartEcg = (RelativeLayout) findView(MResource.getIdByName(MapApplication.mContext, "id", "relativelaouut_start_ecg"));
		...
		btnAdd = (Button) findView(MResource.getIdByName(MapApplication.mContext, "id", "button_add"));

		btnAdd.setVisibility(View.VISIBLE);
		btnAdd.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_history_icon"));
		btnAdd.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				if (event.getAction() == MotionEvent.ACTION_DOWN) {
					// 更改为按下时的背景图片
					v.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_history_icon_2"));
				} else if (event.getAction() == MotionEvent.ACTION_UP) {
					// 改为抬起时的图片
					v.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_history_icon"));
					// v.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_activity_weight")history_icon);
		...
		btnAdd.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.setClass(mActivity, cmccsi.mhealth.portal.sports.ecg.activity.HistoryECGDetailActivity.class);
	...
	private void startTesting() {
		graphInit();
		// rlStartEcg.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_activity_weight")button_start_ecg);
		ecg_level.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_ecg_level2"));
		Rnum = 0;
	...
	public void onDestroyView() {
		...
		if(tgble!=null){
		tgble.stopRealTimeEKG();
		tgble.close();
		tgble.setHandle(null);
		}
	...
	private void dataReceived(int key, Object data) {
		...
		case TGLibEKG.MSG_CARDIOZONE_HEARTRATE:
			Log.i(TAG, "HR: " + data);
			// tv_HR = (TextView)findView(MResource.getIdByName(MapApplication.mContext, "id", "zyqt_activity_weight")textview_HR_BPM);
			if ((Integer) data > 999)
				data = 999;
		...
		case TGLibEKG.MSG_HEART_LEVEL:
			...
			msg5.sendToTarget();
			// tv = (TextView)findView(MResource.getIdByName(MapApplication.mContext, "id", "zyqt_activity_weight")textview_TRAINING_ZONE);
	...
	private void ekgStop(int result, int finalHR) {
		...
		if (recordname != null) {
			...
			String appName = getResources().getString(MResource.getIdByName(MapApplication.mContext, "string", "zyqt_app_name"));
	...
	private void addProgressView(int max, int value) {
		...
		if (mTextViewStepPercent == null)
			mTextViewStepPercent = findView(MResource.getIdByName(MapApplication.mContext, "id", "textview_percent"));
...
//			ecgdata1 = (i % 2 == 0) ? ecgRawData[i] : (-ecgRawData[i]);
	...
	private void graphInit() {
		...
		// findView(MResource.getIdByName(MapApplication.mContext, "id", "zyqt_activity_weight")ecg_view_line);
	 ...
	 * @time 上午10:03:50
	 */
	private void setBleConnectStartState(){
		buttonText.setText("正在连接...");
		rlStartEcg.setEnabled(false);
		rlStartEcg.setBackground(getActivity().getResources().getDrawable(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_btn_ecg_unenable")));
	}
	 ...
	 * @time 上午10:12:36
	 */
	private void setBleConnectSucessState(){
		isConnect=true;
		buttonText.setText("开始测量");
		rlStartEcg.setEnabled(true);
		rlStartEcg.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_button_unstart_ecg"));
	}
	 ...
	 * @time 上午10:21:04
	 */
	private void setBleConnectFailState(){
		isConnect=false;
		buttonText.setText("重新连接");
		rlStartEcg.setEnabled(true);
		rlStartEcg.setBackgroundResource(MResource.getIdByName(MapApplication.mContext, "drawable", "zyqt_button_unstart_ecg"));
	}
