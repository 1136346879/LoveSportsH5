...
public class CenterRollingBall extends View {
	private int[] colorArray = new int[] { Color.rgb(203, 203, 203), Color.rgb(168, 211, 59), Color.rgb(253, 230, 34),
	...
	private void drawEndBall(Canvas canvas) {
		float endBallCX = (float) ((arcRectF.left + arcRectF.right) / 2 + (arcRectF.right - arcRectF.left - centerOffest) / 2
				* Math.cos(Math.PI * 2 * (scorePercent + angleOffest / 360)));
		float endBallCY = (float) ((arcRectF.top + arcRectF.bottom) / 2 + (arcRectF.right - arcRectF.left - centerOffest) / 2
	...
	private void drawCenterCircle(Canvas canvas) {
		...
		paint.setColor(Color.WHITE);
		canvas.drawCircle((arcRectF.left + arcRectF.right) / 2, (arcRectF.top + arcRectF.bottom) / 2,
				(arcRectF.right - arcRectF.left) / 2 - centerOffest, paint);
	}
	...
	private void drawArcs(Canvas canvas) {
		...
		if (sweepAngle > 2 && showFrontArc) {
			frontArcAngle = 1;
			paint.setColor(Color.WHITE);
			canvas.drawArc(arcRectF, 0 + angleOffest, frontArcAngle, true, paint);
		}
		paint.setColor(colorArray[colorIndex + 1]);
		canvas.drawArc(arcRectF, 0 + angleOffest + frontArcAngle, sweepAngle, true, paint);
		paint.setColor(colorArray[colorIndex]);
		canvas.drawArc(arcRectF, 360 * scorePercent + angleOffest, 360 - 360 * scorePercent, true, paint);
	}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		System.out.println("-----------CenterRollingBall-----onTouchEvent------");
